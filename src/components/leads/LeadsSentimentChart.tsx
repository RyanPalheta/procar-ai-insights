...
const SENTIMENT_CONFIG: Record<string, { emoji: string; color: string; label: string }> = {
  "Positivo": { emoji: "😊", color: "hsl(var(--success))", label: "Positivo" },
  "Neutro": { emoji: "😐", color: "hsl(var(--warning))", label: "Neutro" },
  "Negativo": { emoji: "😞", color: "hsl(var(--destructive))", label: "Negativo" }
};

export function LeadsSentimentChart({ data }: LeadsSentimentChartProps) {
  const total = data.reduce((sum, item) => sum + item.value, 0);
  
  // Sort by value to find the dominant sentiment
  const sortedData = [...data].sort((a, b) => b.value - a.value);
  const maxValue = sortedData[0]?.value || 1;

  // Calculate size and opacity based on percentage
