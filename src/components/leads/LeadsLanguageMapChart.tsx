...
const LANGUAGE_CONFIG: Record<string, { flag: string; color: string; label: string; position: { x: number; y: number } }> = {
  "EN-USA": { flag: "🇺🇸", color: "hsl(var(--chart-1))", label: "United States", position: { x: 22, y: 32 } },
  "PT-BR": { flag: "🇧🇷", color: "hsl(var(--chart-2))", label: "Brazil", position: { x: 32, y: 65 } },
  "ES-ES": { flag: "🇪🇸", color: "hsl(var(--chart-3))", label: "Spain", position: { x: 48, y: 30 } },
  "EN-UK": { flag: "🇬🇧", color: "hsl(var(--chart-4))", label: "United Kingdom", position: { x: 46, y: 22 } },
  "FR-FR": { flag: "🇫🇷", color: "hsl(var(--chart-5))", label: "France", position: { x: 49, y: 28 } },
};

const DEFAULT_CONFIG = { flag: "🌐", color: "hsl(var(--muted-foreground))", label: "Other", position: { x: 50, y: 50 } };
        ...
        <CardContent>
          <div className="flex flex-col lg:flex-row gap-4 min-h-[300px]">
            ...
            <div className="flex-1 flex items-center justify-center relative">
              ...
              >
                {/* Map background - more visible */}
                <g className="text-muted-foreground" style={{ opacity: 0.35 }}>
                  {/* North America */}
                  <path
                ...
                {/* Pin markers for each country with data */}
                {sortedData.map((item, index) => {
                  const config = getConfig(item.name);
                  if (!LANGUAGE_CONFIG[item.name]) return null;
                  
                  const x = config.position.x * 10;
                  const y = config.position.y * 5;
