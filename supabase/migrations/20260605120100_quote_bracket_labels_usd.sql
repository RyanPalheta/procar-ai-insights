-- 2026-06-05: Troca os rótulos das faixas de cotação de BRL (R$) para USD ($).
--
-- A function get_conversion_by_quote_bracket() monta os rótulos das faixas
-- ("R$ 0-500", "R$ 500-1000", ...) no próprio Postgres e os retorna como
-- quote_bracket, que o gráfico LeadsConversionByQuoteChart renderiza literalmente
-- no eixo X / tooltip. Como lead_price é em USD, esses rótulos "R$" estão errados.
--
-- Esta migration recria a function com rótulos em "$". É obrigatório atualizar
-- tanto o CASE-WHEN que gera o rótulo quanto o ORDER BY CASE que ordena por ele,
-- senão a ordenação das faixas quebra. Demais lógica permanece idêntica.
