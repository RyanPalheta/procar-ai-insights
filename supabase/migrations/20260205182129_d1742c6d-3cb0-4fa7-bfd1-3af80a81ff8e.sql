...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- Leads ganhos (usando LIKE para capturar variações) - ATUALIZADO para incluir agendamento confirmado
    'won_leads', CASE 
      WHEN period_days IS NOT NULL THEN
        (SELECT COUNT(*) FROM lead_db 
         WHERE last_ai_update IS NOT NULL 
         AND (LOWER(sales_status) LIKE '%ganha%' OR LOWER(sales_status) LIKE '%won%' OR LOWER(sales_status) LIKE '%agendamento confirmado%')
    ...
    -- Leads ganhos período anterior - ATUALIZADO para incluir agendamento confirmado
