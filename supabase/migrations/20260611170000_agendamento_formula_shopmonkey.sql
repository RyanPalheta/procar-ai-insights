-- 2026-06-11 (pedido Pro Car, print 09:59): Conversão de Agendamento =
--   (Agendamentos do ShopMonkey ÷ Total de leads do período) × 100
-- O numerador deixa de ser o status da Kommo (que é snapshot e atrasa) e passa a
-- ser a FONTE REAL da loja: agendamentos green do ShopMonkey por data do
-- agendamento (start_date), a mesma regra do funil por vendedor (green = agendamento).
-- no_show_leads (status "Faltou agendamento" da Kommo) permanece como métrica separada.
-- Única mudança vs 20260611140000: blocos appointment_leads / appointment_leads_previous.
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'sale_leads_previous', CASE
      ...
      ELSE NULL
    END,
    -- AGENDAMENTOS = fonte REAL da loja (ShopMonkey, green = agendamento), por
    -- data do agendamento. Fórmula do card: (agendamentos ÷ total_leads) × 100.
