...
ALTER TABLE lead_db ADD COLUMN IF NOT EXISTS has_upsell BOOLEAN DEFAULT false;
ALTER TABLE lead_db ADD COLUMN IF NOT EXISTS upsell_products TEXT[];
ALTER TABLE lead_db ADD COLUMN IF NOT EXISTS upsell_value_estimate NUMERIC;
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'upsell_leads', CASE 
      WHEN period_days IS NOT NULL THEN
        (SELECT COUNT(*) FROM lead_db WHERE has_upsell = true AND last_ai_update IS NOT NULL AND created_at >= period_start)
      ELSE
    ...
    'upsell_total_value', CASE 
      WHEN period_days IS NOT NULL THEN
        (SELECT COALESCE(SUM(upsell_value_estimate), 0) FROM lead_db WHERE has_upsell = true AND last_ai_update IS NOT NULL AND created_at >= period_start)
      ELSE
