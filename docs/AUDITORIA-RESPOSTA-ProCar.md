...
| **Confiabilidade dos dados (premissa central).** "A maioria dos volumes de leads diverge da base real (Kommo)... pedimos que a equipe documente a fonte e o racional de cálculo." | Fonte e racional de cada indicador documentados nos tooltips; reconciliação Kommo × painel ativa em Configurações (mede o desvio por período). Sincronização horária Kommo→painel (espelho insert-only que já recuperou ~1.086 leads em 30 dias) em operação. Convergência da base segue em acompanhamento. | EM AJUSTE |
...
| Ponto da auditoria (texto original) | Ajuste realizado | Status |
|---|---|---|
| **Leads por Língua.** "Muito impreciso. Deve trazer todos os atendimentos de um período e segmentá-los pelos idiomas... inglês, espanhol e português." | Segmentação por idioma (PT/EN/ES) com % e tooltip, mais "Idiomas por canal" (aba Canais). Hoje o idioma só é gravado quando o chat o informa, cobrindo pouca parte da base; para ficar acurado, ele precisa ser **derivado pela IA** (no analyze-lead) e reprocessado — próximo passo já mapeado. | EM AJUSTE |
| **Leads novos (24h) por canal.** "O número de leads novos não bate com o Kommo: o dashboard apontou 16 no dia; a auditoria encontrou 11. A quebra por canal também diverge." | O backfill de 120 dias trouxe os leads ausentes e o número de 24h confere com a base; os rótulos de canal foram padronizados (Telefone, E-mail, Meta Ads, Indicação, Outros). Falta classificar a **origem real** dos leads importados da Kommo (hoje caem em "Outros") — depende a Pro Car informar o campo/etapa de origem na Kommo. | EM AJUSTE |
| **Taxa de conversão (13,2%).** "Irreal... esclareça se 'conversão' significa venda ganha ou agendamento." | Resolvido com a definição da Pro Car: a Visão Geral mostra **duas conversões separadas** — **Conversão de Venda** (venda ganha) e **Conversão de Agendamento** (agendamento confirmado) — sobre a **base completa** de leads (Kommo), não mais só sobre os ~37% auditados pela IA. Ex. (7 dias): venda **21,6%**, agendamento **9,2%**. | FUNCIONAL |
| **Leads com cotação.** "Número muito abaixo da realidade. A maioria dos leads recebe cotação." | Resolvido: conta a **base completa** — a cotação que a IA extrai do chat **+** o preço registrado na Kommo —, não mais só os leads auditados pela IA. Em 7 dias saltou de 37 → **156** cotações. | FUNCIONAL |
...
| **Gráfico "Leads novos por período".** "A flutuação/constância é razoável, mas os números não são acurados o suficiente." | Resolvido: barras ancoradas ao período selecionado ("Ontem" e intervalos personalizados casam com a data real dos leads) e, com o backfill de 120 dias, o volume diário passou a refletir a base Kommo completa. | FUNCIONAL |
...
| **Distribuição de compliance.** "123 leads auditados em 7 dias parece baixo/insuficiente... Ampliar a cobertura ou apresentar o racional de amostragem." | Racional de amostragem exposto no tooltip e total auditado no cartão ("clientes analisados pela IA"). A ampliação da cobertura depende dos scripts de IA da Pro Car rodarem sobre mais conversas. | EM AJUSTE |
| **Conversão por tempo de resposta.** "Interessante... mas a IA tem dificuldade de identificar o que passou por cotação, e fica confuso se 'conversão' é venda ou agendamento." | Definição de "conversão" resolvida (= **venda**, coerente com a Visão Geral) e parametrizado por período. Segue limitado pela amostra de chat (exige ≥3 mensagens na conversa) e aguarda alinhar o cálculo do tempo ao novo método de 1ª resposta real. | EM AJUSTE |
| **Conversão por cotação.** "Interessante, porém não confiável — os indicadores de conversão não fecham entre si." | Definição de "conversão" resolvida (= **venda**), faixas em USD e filtro por período. Agora os indicadores de conversão usam a mesma regra entre si. A análise por faixa depende de mais cotações registradas (hoje a maioria ainda cai em "Sem Cotação"). | EM AJUSTE |
| **Taxa de objeções contornadas.** "Indicador interessante, mas baseado em apenas 129 atendimentos auditados. Queremos ampliar a amostra." | Base e fórmula expostas no tooltip e o tamanho da amostra no cartão; não há limite fixo — toda a base do período é considerada. A amostra cresce conforme a IA audita mais leads. | EM AJUSTE |
...
| Ponto da auditoria (texto original) | Ajuste realizado | Status |
|---|---|---|
| **Leads de hoje.** "Número impreciso frente ao que de fato chegou. Esclarecer se considera telefone." | Esclarecido na tela: conta só conversas de WhatsApp/chat; as ligações são contadas à parte (cartão "Chamadas") e não entram aqui. A precisão frente ao total ainda depende da convergência da base — por isso o número é menor que o do Kommo. | EM AJUSTE |
...
| **Distribuição por hora.** "Visualização boa... Porém o volume parece baixo demais... double-check da acurácia." | Visualização mantida, com tooltip. O gráfico sobrepõe, por hora, os leads de chat e as ligações telefônicas — não é só o chat. A parte de chat ainda reflete a base em convergência; a de ligações é completa. | EM AJUSTE |
...
| Ponto da auditoria (texto original) | Ajuste realizado | Status |
|---|---|---|
| **Painel-resumo do vendedor.** "Indicadores do resumo não condizem com o dia: Conversão 15% (duvidoso); Cotações (dia) 1 (Ricardo fez dezenas de orçamentos); Objeções 17%." | Painel novo com números reais por vendedor: total de leads (Kommo), orçamentos, agendamentos, taxa de agendamento, vendas, receita e ticket médio (ShopMonkey). "Cotações = 1" virou os orçamentos reais e a "conversão 15%" virou a conversão real (vendas ÷ total de leads, PR #30). O painel de qualidade do chat é mantido como subconjunto, com o aviso "X de Y leads auditados". | EM AJUSTE |
| **Detalhe do vendedor.** "Ao abrir o detalhe, os números mudam e seguem irreais... Meta de conversão 15% (meta 10%)." | Lógica da meta corrigida (compara com o alvo configurado — fim do "15% com meta 10%") e resumo × detalhe agora coerentes. A tela de cadastro de metas existe; falta a Pro Car cadastrar os valores (a seção "Metas" só aparece quando há metas cadastradas). | EM AJUSTE |
| **Timeline dos leads.** "Irreal. Revisar a fonte." | Ancorada ao mesmo universo dos cards (a soma das barras = "X de Y leads auditados pela IA"). O volume acompanha a atribuição de vendedor, que no chat é um subconjunto — o vendedor confiável vive no ShopMonkey. | EM AJUSTE |
...
| Ponto da auditoria (texto original) | Ajuste realizado | Status |
|---|---|---|
| **Leads recebidos por canal.** "Imprecisos e divergentes da base — esta é a raiz... Exceção: o telefone está sendo puxado corretamente." | Telefone mantido correto (vem das ligações reais). E-mail e Indicação — antes descartados — passaram a aparecer; os demais canais foram rotulados como subconjunto de chat ("menor que o Kommo"), com tooltip. Volume em convergência de base. | EM AJUSTE |
| **Volumes por canal (ranking comparativo).** "Útil como ideia, mas com os números errados acima. Pedimos destrinchar: [...] idiomas por canal e quais vendedores têm mais sucesso [...]." | "Idiomas por canal" entregue. "Quais vendedores têm mais sucesso" passou a ser respondido na aba Vendedores (conversão real por vendedor); o cruzamento vendedor × canal segue dependente da atribuição de vendedor na origem. | EM AJUSTE |
