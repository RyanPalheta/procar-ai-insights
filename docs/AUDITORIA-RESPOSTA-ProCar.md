...
| **Confiabilidade dos dados (premissa central).** "A maioria dos volumes de leads diverge da base real (Kommo)... pedimos que a equipe documente a fonte e o racional de cálculo." | Fonte e racional de cada indicador documentados nos tooltips (entregue). A reconciliação Kommo × painel está **visível na Visão Geral** (card "Saúde da base"), medindo o desvio por período. A sincronização horária Kommo→painel (espelho insert-only) e um backfill estão em operação, trazendo os leads ausentes. **Ressalva honesta:** o espelho ainda **não tem deduplicação chat↔Kommo por telefone**, então a base do painel pode conter o mesmo lead duas vezes (linha de chat + espelho Kommo) — a convergência final (dedup) segue em andamento e é medida pelo próprio card. | EM AJUSTE |
...
| Ponto da auditoria (texto original) | Ajuste realizado | Status |
|---|---|---|
| **Leads de hoje.** "Número impreciso frente ao que de fato chegou. Esclarecer se considera telefone." | Esclarecido na tela: as ligações são contadas à parte (cartão "Chamadas") e não entram aqui. O KPI conta a **base do painel** (espelho Kommo + chat) com data de hoje — não é "só chat". **Ressalva honesta:** como o espelho ainda **não foi deduplicado** (chat↔Kommo por telefone), o número pode conter duplicidade até a convergência final da base. | EM AJUSTE |
...
| **Distribuição por hora.** "Visualização boa... Porém o volume parece baixo demais... double-check da acurácia." | Visualização mantida, com tooltip. O gráfico sobrepõe, por hora, os leads de chat e as ligações telefônicas. A série de **ligações é completa**; a de **chat reflete a base do painel**, que ainda está em convergência/deduplicação — por isso o volume de chat pode mudar quando a dedup concluir. | EM AJUSTE |
...
| Ponto da auditoria (texto original) | Ajuste realizado | Status |
|---|---|---|
| **Leads frios sem follow-up / reativáveis.** "Aponta 6% de follow-up adequado, mas audita poucos leads... ampliar a cobertura de auditoria." | Varredura sem amostra fixa (todos os leads frios elegíveis, sem limite) + 4 KPIs com fórmula no tooltip. **Ressalva honesta:** (1) a cobertura = os leads frios já **auditados pela IA**, que cresce com o pipeline; (2) hoje os 4 KPIs são calculados sobre **toda a base** (ainda não filtram o período selecionado) — ajuste pequeno pendente. | EM AJUSTE |
...
| **Volumes por canal (ranking comparativo).** "Útil como ideia, mas com os números errados acima. Pedimos destrinchar: [...] idiomas por canal e quais vendedores têm mais sucesso [...]." | "Idiomas por canal" entregue. Faltam dois pontos: (1) o cruzamento **vendedor × canal** ("quais vendedores têm mais sucesso"), bloqueado pela atribuição de vendedor na origem do chat/Kommo (genérica em ~99% — o vendedor confiável só existe no ShopMonkey, sem o canal amarrado); (2) as colunas de **volume/conversão do comparativo** ainda são lidas sobre a amostra de chat (rótulo "menor que o Kommo"), faltando migrá-las para a base por `source_id` já usada em "Leads recebidos por canal". | EM AJUSTE |
