...
| Ponto da auditoria (texto original) | Ajuste realizado | Status |
|---|---|---|
| **Leads por Língua.** "Muito impreciso. Deve trazer todos os atendimentos de um período e segmentá-los pelos idiomas... inglês, espanhol e português." | Segmentação por idioma (PT/EN/ES e demais) com % por língua e tooltip de fonte, mais a visão "Idiomas por canal" (aba Canais). O idioma é identificado pela IA em ~52% das conversas (amostra); a precisão acompanha a convergência da base. | EM AJUSTE |
| **Leads novos (24h) por canal.** "O número de leads novos não bate com o Kommo: o dashboard apontou 16 no dia; a auditoria encontrou 11. A quebra por canal também diverge." | A sincronização Kommo→painel (insert-only) trouxe os leads ausentes e aproximou o total da Kommo. A normalização de canal está completa para WhatsApp/Facebook/Instagram; os canais vindos da Kommo (Meta Ads, E-mail, Indicação) ainda estão em ajuste de rótulo. Em convergência de base. | EM AJUSTE |
| **Taxa de conversão (13,2%).** "Irreal... esclareça se 'conversão' significa venda ganha ou agendamento." | Regra explicitada no tooltip (hoje soma "venda ganha" e "agendamento confirmado"). A aba Vendedores já traz os dois separados — conversão de venda e taxa de agendamento (PR #30); falta a **definição de negócio** para unificar na Visão Geral. | EM AJUSTE |
| **Leads com cotação.** "Número muito abaixo da realidade. A maioria dos leads recebe cotação." | Passou a considerar o preço vindo da Kommo, além da cotação que a IA extrai do chat, ampliando a base. A sincronização é incremental (não reescreve preços antigos), então segue subcontando até a base convergir. | EM AJUSTE |
...
| **Gráfico "Leads novos por período".** "A flutuação/constância é razoável, mas os números não são acurados o suficiente." | Alinhamento de datas corrigido: as barras passaram a ser ancoradas ao período selecionado (não mais à data de hoje), fazendo "Ontem" e intervalos personalizados casarem com a data real dos leads. A acurácia do volume diário acompanha a convergência da base. | EM AJUSTE |
...
| **Conversão por tempo de resposta.** "Interessante... mas a IA tem dificuldade de identificar o que passou por cotação, e fica confuso se 'conversão' é venda ou agendamento." | Parametrizado por período, com tooltip. Dois ajustes seguem em aberto: alinhar o cálculo do tempo ao novo método de 1ª resposta real (cliente → agente) e a definição de "conversão" (hoje soma venda e agendamento). | EM AJUSTE |
| **Conversão por cotação.** "Interessante, porém não confiável — os indicadores de conversão não fecham entre si." | Faixas em USD e filtro por período entregues, com tooltip. A coerência entre os indicadores de conversão depende da definição única de "conversão" (hoje o gráfico soma venda e agendamento sobre a base de chat); a separação venda × agendamento já existe por vendedor. | EM AJUSTE |
